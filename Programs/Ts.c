...
int main(int argc, char ** argv){
  ...
  int ithread;
  float *Tk_box, *x_e_box, *Lya_box, *Ts, J_star_Lya, dzp, prev_zp, zpp, prev_zpp, prev_R;
...
/***************  END PARALLELIZED LOOP ******************************************************************/
  ...
  //if(USE_GENERAL_SOURCES) sum_lyn[R_ct] += frecycle(n_ct);
	/*else*/ sum_lyn[R_ct] += frecycle(n_ct) * spectral_emissivity(nuprime, 0, Pop, src.fPopIII(zpp), zpp);
      }
...
#pragma omp for
    ...
    for (box_ct=0; box_ct<HII_TOT_NUM_PIXELS; box_ct++){
			//fprintf(stderr, "Thread num: %f\n", omp_get_thread_num());
			//fprintf(LOG, "Thread num: %f\n", omp_get_thread_num());
	...
	Tk_box[box_ct] += dansdz[1] * dzp;
      ...
      if (Tk_box[box_ct]<0){ // spurious bahaviour of the trapazoidal integrator. generally overcooling in underdensities
	...
	//Lya_box[box_ct] = J_alpha_tot * hplank * Ly_alpha_HZ / (1+zp);
...
/***************  END PARALLELIZED LOOP ******************************************************************/
	...
	if (mod_fwrite(x_e_box, sizeof(float)*HII_TOT_NUM_PIXELS, 1, F)!=1){
	  fprintf(stderr, "Ts.c: Write error occured while writing x_e box.\n");
	  fprintf(LOG, "Ts.c: Write error occured while writing x_e box.\n");
	}
	fclose(F);
      }
    }
  ...
  if ( COMPUTE_Ts ){
	...
	if (mod_fwrite(Ts, sizeof(float)*HII_TOT_NUM_PIXELS, 1, F)!=1){
	  fprintf(stderr, "Ts.c: Write error occured while writing Ts box.\n");
	  fprintf(LOG, "Ts.c: Write error occured while writing Ts box.\n");
	}
	fclose(F);
      }
