...
static double log10_overdense_spline_SFRPOP2PARK[NSFR_low], log10_Fcoll_spline_SFRPOP2PARK[NSFR_low]; //-GS
...
static gsl_interp_accel *FcollLow_spline_acc_SFRPOP2PARK; //-GS
...
static gsl_spline *FcollLow_spline_SFRPOP2PARK; //-GS
...
void FcollSpline_SFRPOP2PARK(float Overdensity, float *splined_value);
...
void initialiseFcollSFRPOP2PARK_spline(float z, float Mmax, float MassTurnover, float Alpha_star, float Fstar10, float Mlim_Fstar);
...
float *Overdense_spline_SFRPOP2PARK,*Fcoll_spline_SFRPOP2PARK,*second_derivs_SFRPOP2PARK; //-GS
...
float FgtrConditionallnM_GL_SFRPOP2PARK(float lnM, struct parameters_gsl_SFR_con_int_ parameters_gsl_SFR_con); //-GS
...
float GaussLegendreQuad_FcollSFRPOP2PARK(int n, float z, float M2, float delta1, float delta2, float MassTurnover, float Alpha_star, float Fstar10, float Mlim_Fstar); // -GS
...
double dFgtrConditionallnM_SFRPOP2PARK(double lnM, void *params);
...
double FgtrConditionalM_SFRPOP2PARK(double z, double M1, double M2, double delta1, double delta2, double MassTurnover, double Alpha_star, double Fstar10, double Mlim_Fstar); // -GS
...
double dFdlnM_st_SFRPOP2PARK (double lnM, void *params); //-GS
...
double FgtrM_st_SFRPOP2PARK(double z, double MassTurnover, double Alpha_star, double Fstar10, double Mlim_Fstar); // -GS
...
/*
 FUNCTION FgtrM_st_SFRPOP2PARK(z, Mturn)
 */
double dFdlnM_st_SFRPOP2PARK(double lnM, void *params){
  struct parameters_gsl_SFR_int_ vals = *(struct parameters_gsl_SFR_int_ *)params;
  double M = exp(lnM);
  float z = vals.z_obs;
  double MassTurnover = vals.Mdrop;
  ...
  if(USE_GENERAL_SOURCES)
  {
    sources src;
    src = defaultSources();
    return dNdM_st(z, M) * M * M * f_baryon * src.fstar(z, M) / (t_STAR / hubble(z) * SperYR);
  }
    return 0;
}

double FgtrM_st_SFRPOP2PARK(double z, double MassTurnover, double Alpha_star, double Fstar10, double Mlim_Fstar){
  double M_Min = MassTurnover/50.;
  double result, error, lower_limit, upper_limit;
  gsl_function F;
  double rel_tol = 0.001; //<- relative tolerance
  ...
  F.function = &dFdlnM_st_SFRPOP2PARK;
  F.params = &parameters_gsl_SFR;
  lower_limit = log(M_Min);
  upper_limit = log(FMAX(1e15, M_Min*100));
...
float FgtrConditionallnM_GL_SFRPOP2PARK(float lnM, struct parameters_gsl_SFR_con_int_ parameters_gsl_SFR_con){
    float M = exp(lnM);
    float z = parameters_gsl_SFR_con.z_obs;
    float M2 = parameters_gsl_SFR_con.Mval;
    float del1 = parameters_gsl_SFR_con.delta1;
    ...
    if(USE_GENERAL_SOURCES)
    {
      sources src;
      src = defaultSources();
      return dNdM_conditional_second(z,log(M),M2,del1,del2)/sqrt(2.*PI) * M * f_baryon * src.fstar(z, M) / (t_STAR / hubble(z) * SperYR);
    }
...
float GaussLegendreQuad_FcollSFRPOP2PARK(int n, float z, float M2, float delta1, float delta2, float MassTurnover, float Alpha_star, float Fstar10, float Mlim_Fstar) {
    //Performs the Gauss-Legendre quadrature.
    int i;
    ...
    if(delta2 > delta1){
        ...
        for(i=1; i<(n+1); i++){
            x = xi_SFR[i];
            x = xi_SFR[i];
            integrand += wi_SFR[i]*FgtrConditionallnM_GL_SFRPOP2PARK(x,parameters_gsl_SFR_con);
        }
        return integrand;
    }
...
double FgtrConditionalM_SFRPOP2PARK(double z, double M1, double M2, double delta1, double delta2, double MassTurnover, double Alpha_star, double Fstar10, double Mlim_Fstar) {
    double result, error, lower_limit, upper_limit;
    gsl_function F;
    double rel_tol = 0.005; //<- relative tolerance
    gsl_integration_workspace * w
    ...
    F.function = &dFgtrConditionallnM_SFRPOP2PARK;
    F.params = &parameters_gsl_SFR_con;
    lower_limit = M1;
    upper_limit = M2;
...
// Find alternative SFRD in bathtub model
void initialiseFcollSFRPOP2PARK_spline(float z, float Mmax, float MassTurnover, float Alpha_star, float Fstar10, float Mlim_Fstar){
	float Mmin = MassTurnover/50.;
  if(USE_GENERAL_SOURCES)
  {
    ...
    double overdense_small_high = 1.5, overdense_small_low = -1. + 9e-8;
    int i;
    FcollLow_spline_acc_SFRPOP2PARK = gsl_interp_accel_alloc ();
    FcollLow_spline_SFRPOP2PARK = gsl_spline_alloc (gsl_interp_cspline, NSFR_low);
    for (i=0; i<NSFR_low; i++){
        overdense_val = log10(1. + overdense_small_low) + (double)i/((double)NSFR_low-1.)*(log10(1.+overdense_small_high)-log10(1.+overdense_small_low));

        log10_overdense_spline_SFRPOP2PARK[i] = overdense_val;
        log10_Fcoll_spline_SFRPOP2PARK[i] = log10(GaussLegendreQuad_FcollSFRPOP2PARK(NGL_SFR, z, log(Mmax), Deltac, pow(10.,overdense_val)-1., MassTurnover, Alpha_star, Fstar10, Mlim_Fstar));

        if(log10_Fcoll_spline_SFRPOP2PARK[i] < -40.){
            log10_Fcoll_spline_SFRPOP2PARK[i] = -40.;
        }
    }
    gsl_spline_init(FcollLow_spline_SFRPOP2PARK, log10_overdense_spline_SFRPOP2PARK, log10_Fcoll_spline_SFRPOP2PARK, NSFR_low);

    for(i=0;i<NSFR_high;i++) {
        Overdense_spline_SFRPOP2PARK[i] = overdense_large_low + (float)i/((float)NSFR_high-1.)*(overdense_large_high - overdense_large_low);
        Fcoll_spline_SFRPOP2PARK[i] = FgtrConditionalM_SFRPOP2PARK(z, log(Mmin), log(Mmax), Deltac, Overdense_spline_SFRPOP2PARK[i], MassTurnover, Alpha_star, Fstar10, Mlim_Fstar);
        if(Fcoll_spline_SFRPOP2PARK[i]<0.) {
            Fcoll_spline_SFRPOP2PARK[i]=pow(10.,-40.0);
        }
    }
    spline(Overdense_spline_SFRPOP2PARK-1, Fcoll_spline_SFRPOP2PARK-1, NSFR_high, 0, 0, second_derivs_SFRPOP2PARK-1);
}
...
void FcollSpline_SFRPOP2PARK(float Overdensity, float *splined_value){
    ...
    if(Overdensity<1.5) {
        if(Overdensity<-1.) {
            ...
            returned_value = gsl_spline_eval(FcollLow_spline_SFRPOP2PARK, log10(Overdensity+1.), FcollLow_spline_acc_SFRPOP2PARK);
            returned_value = pow(10.,returned_value);
        }
    }
    else {
		...
		// However, such densities should always be collapsed, so just set f_coll to unity.
		// Additionally, the fraction of points in this regime relative to the entire simulation volume is extremely small.
            splint(Overdense_spline_SFRPOP2PARK-1, Fcoll_spline_SFRPOP2PARK-1, second_derivs_SFRPOP2PARK-1, NSFR_high, Overdensity, &(returned_value));
        }
        else {
            // We set overdensity of regions close to critical density to critical density to avoid instability.
            //returned_value = 1.;
