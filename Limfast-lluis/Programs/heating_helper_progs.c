...
float *second_derivs_FcollXray_zpp[NUM_FILTER_STEPS_FOR_Ts], *second_derivs_FcollLya_zpp[NUM_FILTER_STEPS_FOR_Ts]; // New
float *redshift_interp_table;
int Nsteps_zp; //New in v1.4
float *zpp_interp_table; //New in v1.4
gsl_interp_accel *FcollLowXray_zpp_spline_acc[NUM_FILTER_STEPS_FOR_Ts];
gsl_spline *FcollLowXray_zpp_spline[NUM_FILTER_STEPS_FOR_Ts];
...
/* returns the spectral emissity */
double spectral_emissivity_alt(double nu_norm, int flag, int stellar_pop, double fPopIII, double cosmic_Z);
...
void evolveInt(float zp, float curr_delNL0[], double freq_int_heat[],
	       ...
	       int COMPUTE_Ts, double y[], double deriv[], int arr_num, double norm_xray, double norm_lya);
...
int init_heat()
{
  ...
  if (spectral_emissivity(0,1,2,0) < 0)
    return -6;
  if (spectral_emissivity_alt(0,1,2,0,0) < 0)
    return -7;
...
void destruct_heat()
{
  ...
  spectral_emissivity_alt(0.0, 2, 2, 0, 0);
}
...
/* Reads in and constructs table of the piecewise power-law fits to Pop 2 and
 * Pop 3 stellar spectra, from BPASS */
double spectral_emissivity_alt(double nu_norm, int flag, int stellar_pop, double fPopIII, double cosmic_Z)
{
  static int n[NSPEC_MAX];
  static float nu_n[NSPEC_MAX], alpha_S_2[NSPEC_MAX];
  ...
  int i;
  size_t ii;
  char ss_filename[1000];
  FILE *F;
  // metallicity values of available SED tables
  float Zvals[] = {1.0e-5, 1.0e-4, 1.0e-3, 2.0e-3, 3.0e-3, 4.0e-3, 6.0e-3, 8.0e-3, 1.0e-2, 1.4e-2, 2.0e-2, 3.0e-2, 4.0e-2};
  float closest_Zval = 1.0e-5;
  //use average to find pop2 SED with closest metallicity and read value from there
  float smallest_diff = fabs(cosmic_Z - Zvals[0]);
  for (ii = 0; ii < sizeof(Zvals) / sizeof(Zvals[0]); ii++) {
    if (fabs(cosmic_Z - Zvals[ii]) < smallest_diff) {
        smallest_diff = fabs(cosmic_Z - Zvals[ii]);
        closest_Zval = Zvals[ii];
    }
  }
  sprintf(ss_filename, "../External_tables/stellar_spectra_bpass/stellar_spectra_Z%.1e.dat", closest_Zval);
  //printf("CHECK closest_Zval:", closest_Zval);
  //printf("CHECK: ../External_tables/stellar_spectra_bpass/stellar_spectra_Z%.1e.dat", closest_Zval);
  if (flag == 1) {
    /* Read in the data */
    //if (!(F = fopen(STELLAR_SPECTRA_FILENAME, "r"))){
    if (!(F = fopen(ss_filename, "r"))){
      fprintf(stderr, "spectral_emissivity_alt: Unable to open file: stellar_spectra.dat for reading\nAborting\n");
      fprintf(LOG, "spectral_emissivity_alt: Unable to open file: stellar_spectra.dat for reading\nAborting\n");
      return -1;
    }
    for (i=1;i<NSPEC_MAX;i++) {
      fscanf(F, "%i %e %e %e %e", &n[i], &N0_2[i], &alpha_S_2[i], &N0_3[i], &alpha_S_3[i]);
  ...
  for (zpp_ct = 0; zpp_ct < NUM_FILTER_STEPS_FOR_Ts; zpp_ct++){
    ...
    if (HALO_MASS_DEPENDENT_IONIZING_EFFICIENCY) {
      ...
      fcollLya = 0;
          ...
          fcoll = gsl_spline_eval(FcollLowXray_zpp_spline[zpp_ct], log10(curr_delNL0[zpp_ct]*growth_zpp+1.), FcollLowXray_zpp_spline_acc[zpp_ct]);
          fcoll= pow(10., fcoll);
	  ...
	  //dfcoll = ST_over_PS[zpp_ct]*(double)fcoll*hubble(zpp)/T_AST*fabs(dtdz(zpp))*fabs(dzpp);
		dfcoll = ST_over_PS[zpp_ct]*(double)fcoll*fabs(dtdz(zpp))*fabs(dzpp);
    //dfcollLya = ST_over_PS_Lya[zpp_ct]*(double)fcollLya*hubble(zpp)/T_AST*fabs(dtdz(zpp))*fabs(dzpp);
		dfcollLya = ST_over_PS_Lya[zpp_ct]*(double)fcollLya*fabs(dtdz(zpp))*fabs(dzpp);
	}
	else {
