...
int main(int argc, char ** argv){
  ...
  for (i=0; i<HII_DIM; i++){
	...
	if (USE_TS_IN_21CM){
      if(SUBCELL_RSD) {
          ...
          //printf("\n- CHECK COORD: %d, %d, %d", i, j, k);
          //printf("- CHECK PIXEL: %e, %e\n", const_factor*pixel_x_HI*(1+pixel_deltax), pixel_Ts_factor);
  ...
  sprintf(filename, "../Boxes/delta_T_RSD_v%i_z%06.2f_%i_%.0fMpc_maxdvdr%06.2f", VELOCITY_COMPONENT, REDSHIFT, HII_DIM, BOX_LEN, MAX_DVDR);
  F = fopen(filename, "wb");
...
// deallocate what we aren't using anymore
 ...
 k_factor = 1.5;
  ...
  if (T_USE_VELOCITIES){
    sprintf(filename, "%s/ps_21cm_RSD_z%06.2f_aveTb%06.2f_%i_%.0fMpc_maxdvdr%06.2f", psoutputdir, REDSHIFT,  ave, HII_DIM, BOX_LEN, MAX_DVDR);
  }
  else{
    sprintf(filename, "%s/ps_21cm_z%06.2f_aveTb%06.2f_%i_%.0fMpc_maxdvdr%06.2f", psoutputdir, REDSHIFT,  ave, HII_DIM, BOX_LEN, MAX_DVDR);
  }
  F = fopen(filename, "w");
  if (!F){
