...
float *RHOL_HI6563A;
float *RHOL_OII3726A;
float *RHOL_OIII5007A;
float *RHOL_HI4861A;
float *RHOL_CII158m;
float *RHOL_CO2600m;
float *I_HI1216A_box;
float *I_HI6563A_box;
float *I_OII3726A_box;
float *I_OIII5007A_box;
float *I_HI4861A_box;
float *I_CII158m_box;
float *I_CO2600m_box;
...
void init_21cmMC_arrays() { // defined in Cosmo_c_files/ps.c
    ...
    Overdense_spline_gas = calloc(NSFR_high,sizeof(float));      // - LMR
    Overdense_spline_met = calloc(NSFR_high,sizeof(float));      // - LMR
    ...
    Overdense_spline_LINELUMPOP2F = calloc(NSFR_high,sizeof(float)); //-GS
    Overdense_spline_LINELUMPOP2G = calloc(NSFR_high,sizeof(float)); //-GS
    Fcoll_spline_SFR = calloc(NSFR_high,sizeof(float));
    Fcoll_spline_SFRPOP2 = calloc(NSFR_high,sizeof(float)); //-GS
    Fcoll_spline_gas = calloc(NSFR_high,sizeof(float));      // - LMR
    Fcoll_spline_met = calloc(NSFR_high,sizeof(float));      // - LMR
    ...
    Fcoll_spline_LINELUMPOP2F = calloc(NSFR_high,sizeof(float)); //-GS
    Fcoll_spline_LINELUMPOP2G = calloc(NSFR_high,sizeof(float)); //-GS
    ...
    second_derivs_gas = calloc(NSFR_high,sizeof(float));      // - LMR
    second_derivs_met = calloc(NSFR_high,sizeof(float));      // - LMR
    ...
    second_derivs_LINELUMPOP2F = calloc(NSFR_high,sizeof(float)); //-GS
    second_derivs_LINELUMPOP2G = calloc(NSFR_high,sizeof(float)); //-GS
...
void destroy_21cmMC_arrays() {
    ...
    free(Overdense_spline_gas);      // - LMR
    free(Overdense_spline_met);      // - LMR
    ...
    free(Overdense_spline_LINELUMPOP2F); //-GS
    free(Overdense_spline_LINELUMPOP2G); //-GS
    free(Fcoll_spline_SFR);
    free(Fcoll_spline_SFRPOP2); //-GS
    free(Fcoll_spline_gas);     // - LMR
    free(Fcoll_spline_met);      // - LMR
    ...
    free(Fcoll_spline_LINELUMPOP2F); //-GS
    free(Fcoll_spline_LINELUMPOP2G); //-GS
    if(USE_GENERAL_SOURCES)
    {
    ...
    free(second_derivs_gas);      // - LMR
    free(second_derivs_met);      // - LMR
    ...
    free(second_derivs_LINELUMPOP2F); //-GS
    free(second_derivs_LINELUMPOP2G); //-GS
    free(xi_SFR);
    free(wi_SFR);
}
...
int main(int argc, char ** argv){
  ...
  float Splined_gas; // -GS
  float Splined_met; // -GS
  //printf("CHECK splined met: %e\n", Splined_met);
  float Splined_RHOL_HI1216A, Splined_RHOL_HI6563A, Splined_RHOL_HI4861A, Splined_RHOL_CII158m, Splined_RHOL_CO2600m,Splined_RHOL_OII3726A,Splined_RHOL_OIII5007A;
  double mean_sfrd_st, ST_over_PS_SFRD, sfrd; // scalar set for SFRD -GS
  double mean_gas_st, ST_over_PS_gas, gass; // scalar set for SFRD -GS
  double mean_met_st, ST_over_PS_met, mets; // scalar set for SFRD -GS
  ...
  double mean_rhol_st_OII3726A, ST_over_PS_OII3726A, rhol_oii3726a; // scalar set for Ha luminosity density -GS
  double mean_rhol_st_OIII5007A, ST_over_PS_OIII5007A, rhol_oiii5007a; // scalar set for Ha luminosity density -GS
...
/*  if ((fabs(ALPHA_STAR) > FRACT_FLOAT_ERR) ||
  ...
  gas = (float *) malloc(sizeof(float)*HII_TOT_FFT_NUM_PIXELS);
  gasbox = (float *) malloc(sizeof(float)*HII_TOT_FFT_NUM_PIXELS);
  met = (float *) malloc(sizeof(float)*HII_TOT_FFT_NUM_PIXELS);
  metbox = (float *) malloc(sizeof(float)*HII_TOT_FFT_NUM_PIXELS);
  ...
  RHOL_OII3726A = (float *) malloc(sizeof(float)*HII_TOT_FFT_NUM_PIXELS);
  RHOL_OIII5007A = (float *) malloc(sizeof(float)*HII_TOT_FFT_NUM_PIXELS);
  ...
  I_OII3726A_box = (float *) malloc(sizeof(float)*HII_TOT_FFT_NUM_PIXELS);
  I_OIII5007A_box = (float *) malloc(sizeof(float)*HII_TOT_FFT_NUM_PIXELS);
 ...
 // }
  ...
  mean_gas_st = FgtrM_st_gas(REDSHIFT, M_TURN, ALPHA_STAR, F_STAR10, Mlim_Fstar);
  //printf("meang_as_st: %e\n", mean_gas_st);
  mean_met_st = FgtrM_st_met(REDSHIFT, M_TURN, ALPHA_STAR, F_STAR10, Mlim_Fstar);
  //printf("mean_metallicity_st: %e\n", mean_met_st);
  ...
  mean_rhol_st_OII3726A = FgtrM_st_LINELUMPOP2F(REDSHIFT, M_TURN, ALPHA_STAR, F_STAR10, Mlim_Fstar, 5);
  mean_rhol_st_OIII5007A = FgtrM_st_LINELUMPOP2G(REDSHIFT, M_TURN, ALPHA_STAR, F_STAR10, Mlim_Fstar, 4);


  //fprintf(stderr, "Ha  %e\n ", mean_rhol_st_HI6563A);
  //fprintf(stderr, "OII  %e\n ", mean_rhol_st_OII3726A);
  //fprintf(stderr, "OIII  %e\n ", mean_rhol_st_OIII5007A);
 ...
 CLEANUP: if (*error_message != '\0'){
	...
	//printf(LOG, "begin filter, clock=%06.2f\n", (double)clock()/CLOCKS_PER_SEC);
        fflush(LOG);
    ...
    initialiseFcollgas_spline(REDSHIFT, massofscaleR, M_TURN, ALPHA_STAR, F_STAR10, Mlim_Fstar);
    initialiseFcollmet_spline(REDSHIFT, massofscaleR, M_TURN, ALPHA_STAR, F_STAR10, Mlim_Fstar);
    ...
    initialiseFcollLINELUMPOP2F_spline(REDSHIFT, massofscaleR, M_TURN, ALPHA_STAR, F_STAR10, Mlim_Fstar, 5);
    initialiseFcollLINELUMPOP2G_spline(REDSHIFT, massofscaleR, M_TURN, ALPHA_STAR, F_STAR10, Mlim_Fstar, 4);
    // - RM
    /*
    if(USE_GENERAL_SOURCES)
    {
    ...
    if(USE_GENERAL_SOURCES)
    {
      ...
      Splined_Fcoll_uni = splined_erfc(erfc_num/erfc_denom);
        ...
        FcollSpline_gas(density_over_mean - 1, &(Splined_gas));
        FcollSpline_met(density_over_mean - 1, &(Splined_met));
        ...
        FcollSpline_LINELUMPOP2F(density_over_mean - 1, &(Splined_RHOL_OII3726A));
        FcollSpline_LINELUMPOP2G(density_over_mean - 1, &(Splined_RHOL_OIII5007A));
        }
      ...
      Splined_Fcoll_uni = 1.0;
        ...
        gas[HII_R_FFT_INDEX(x,y,z)] = Splined_gas * (OMm*RHOcrit);
        met[HII_R_FFT_INDEX(x,y,z)] = Splined_met * (OMm*RHOcrit);
        ...
        RHOL_OII3726A[HII_R_FFT_INDEX(x,y,z)] = Splined_RHOL_OII3726A * (OMm*RHOcrit);
        RHOL_OIII5007A[HII_R_FFT_INDEX(x,y,z)] = Splined_RHOL_OIII5007A * (OMm*RHOcrit);
        ...
        rhol_oii3726a += Splined_RHOL_OII3726A;
        rhol_oiii5007a += Splined_RHOL_OIII5007A;
  ...
  gass /= (double) HII_TOT_NUM_PIXELS; // ave PS fcoll for this filter scale
  mets /= (double) HII_TOT_NUM_PIXELS; // ave PS fcoll for this filter scale
  //printf("average gass : %e\n", gass);
  rhol_hi1216a /= (double) HII_TOT_NUM_PIXELS;
  rhol_hi6563a /= (double) HII_TOT_NUM_PIXELS;
  rhol_oii3726a /= (double) HII_TOT_NUM_PIXELS;
  rhol_oiii5007a /= (double) HII_TOT_NUM_PIXELS;
  ...
  ST_over_PS_gas = mean_gas_st / gass;
  ST_over_PS_met = mean_met_st / mets;
  ST_over_PS_HI1216A = mean_rhol_st_HI1216A / rhol_hi1216a;
  ST_over_PS_HI6563A = mean_rhol_st_HI6563A / rhol_hi6563a;
  ST_over_PS_OII3726A = mean_rhol_st_OII3726A / rhol_oii3726a;
  ST_over_PS_OIII5007A = mean_rhol_st_OIII5007A / rhol_oiii5007a;
  ...
  ST_over_PS_CO2600m = mean_rhol_st_CO2600m / rhol_co2600m;
    ...
    for (x=0; x<HII_DIM; x++){
      for (y=0; y<HII_DIM; y++){
        for (z=0; z<HII_DIM; z++){
          ...
          gasbox[ct] = gas[ct] * ST_over_PS_gas; // get normalized gas -GS
          metbox[ct] = met[ct] * ST_over_PS_met; // get normalized met -GS
          //printf("CHECK: %e\n", RHOL_HI1216A[ct]);
          ...
          //I_OII3726A_box[ct] = C * Lsun * RHOL_OII3726A[ct] * ST_over_PS_OII3726A / (4 * PI * OII_3727_HZ * hubble(REDSHIFT) * pow(CMperMPC, 3));
          //I_OIII5007A_box[ct] = C * Lsun * RHOL_OIII5007A[ct] * ST_over_PS_OIII5007A / (4 * PI * OIII_5007_HZ * hubble(REDSHIFT) * pow(CMperMPC, 3));
          ...
          //I_HI1216AdIGM_box[ct] = (C * Lya_emissivity) / (4 * PI * Ly_alpha_HZ * hubble(REDSHIFT));
        }
      }
    }
    ...
    fprintf(stderr, "Redshift : %e, mean Lya per cell: %e\n", REDSHIFT, perhaloha);*/
    ...
    //fprintf(stderr, "redshift: %e, Mean xH: %e\n", REDSHIFT, global_xH);
    ...
    sprintf(filename, "../Boxes/gas_mass_z%06.2f_HIIfilter%i_RHIImax%.0f_%i_%.0fMpc", REDSHIFT, HII_FILTER, MFP, HII_DIM, BOX_LEN);
    if (!(F = fopen(filename, "wb"))) {
      sprintf(error_message, "find_HII_bubbles.c: ERROR: unable to open file for writing gas box.\n");
      goto CLEANUP;
    }
    else {
      for (x = 0; x < HII_DIM; x++) {
        for (y = 0; y < HII_DIM; y++) {
          for (z = 0; z < HII_DIM; z++) {
            if (fwrite(gasbox + HII_R_FFT_INDEX(x,y,z), sizeof(float), 1, F) != 1) {
               sprintf(error_message, "find_HII_bubbles.c: Write error occured while writing gas box.\n");
               goto CLEANUP;
            }
          }
        }
    ...
    sprintf(filename, "../Boxes/metal_mass_z%06.2f_HIIfilter%i_RHIImax%.0f_%i_%.0fMpc", REDSHIFT, HII_FILTER, MFP, HII_DIM, BOX_LEN);
    if (!(F = fopen(filename, "wb"))) {
      sprintf(error_message, "find_HII_bubbles.c: ERROR: unable to open file for writing metal mass box.\n");
      goto CLEANUP;
    }
    else {
      for (x = 0; x < HII_DIM; x++) {
        for (y = 0; y < HII_DIM; y++) {
          for (z = 0; z < HII_DIM; z++) {
            if (fwrite(metbox + HII_R_FFT_INDEX(x,y,z), sizeof(float), 1, F) != 1) {
               sprintf(error_message, "find_HII_bubbles.c: Write error occured while writing metal mass box.\n");
               goto CLEANUP;
            }
          }
        }
    ...
    // OII3726A line intensity box
    sprintf(filename, "../Boxes/I_OII3726A_z%06.2f_HIIfilter%i_RHIImax%.0f_%i_%.0fMpc", REDSHIFT, HII_FILTER, MFP, HII_DIM, BOX_LEN);
    if (!(F = fopen(filename, "wb"))) {
      sprintf(error_message, "find_HII_bubbles.c: ERROR: unable to open file for writing OII3726A line intensity box.\n");
      goto CLEANUP;
    }
    else {
      for (x = 0; x < HII_DIM; x++) {
        for (y = 0; y < HII_DIM; y++) {
          for (z = 0; z < HII_DIM; z++) {
            if (fwrite(I_OII3726A_box + HII_R_FFT_INDEX(x,y,z), sizeof(float), 1, F) != 1) {
               sprintf(error_message, "find_HII_bubbles.c: Write error occured while writing OII3726A line intensity box.\n");
               goto CLEANUP;
            }
          }
        }
    ...
    // OIII5007A line intensity box
    sprintf(filename, "../Boxes/I_OIII5007A_z%06.2f_HIIfilter%i_RHIImax%.0f_%i_%.0fMpc", REDSHIFT, HII_FILTER, MFP, HII_DIM, BOX_LEN);
    if (!(F = fopen(filename, "wb"))) {
      sprintf(error_message, "find_HII_bubbles.c: ERROR: unable to open file for writing OIII5007A line intensity box.\n");
      goto CLEANUP;
    }
    else {
      for (x = 0; x < HII_DIM; x++) {
        for (y = 0; y < HII_DIM; y++) {
          for (z = 0; z < HII_DIM; z++) {
            if (fwrite(I_OIII5007A_box + HII_R_FFT_INDEX(x,y,z), sizeof(float), 1, F) != 1) {
               sprintf(error_message, "find_HII_bubbles.c: Write error occured while writing OIII5007A line intensity box.\n");
               goto CLEANUP;
            }
          }
        }
  ...
  if (INHOMO_RECO){
    ...
    if (!(F = fopen(filename, "wb"))){
        ...
        fprintf(LOG, "Neutral fraction is %f\nNow writting xH box at %s\n", global_xH, filename);
        //printf(stderr, "Neutral fraction is %f\nNow writting xH box at %s\n", global_xH, filename);
        fflush(LOG);
        if (mod_fwrite(xH, sizeof(float)*HII_TOT_NUM_PIXELS, 1, F)!=1){
            fprintf(stderr, "find_HII_bubbles.c: Write error occured while writting xH box.\n");
            fprintf(LOG, "find_HII_bubbles.c: Write error occured while writting xH box.\n");
