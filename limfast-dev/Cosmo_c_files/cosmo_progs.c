...
double mean_rho_comoving(float z);
...
double f_star(); /* returns the f_star constant given the mass of some halo */
...
/* returns the comoving mean baryonic density at z in Msun/Mpc^3 */
double mean_rho_comoving(float z){
  return pow(CMperMPC, 3) / Msun * OMb*(3.0*Ho*Ho / (8.0*PI*G));
}
...
// -MG
double f_star() {
  /*double f = STELLAR_BARYON_FRAC * pow(M_h / (pow(10, 10) * Msun), STELLAR_BARYON_PL);
  return f < 1.0 ? f : 1.0;*/
  return 0.05;
}
