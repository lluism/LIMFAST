...
for path in args.input:
    ...
    # check if it is an Fcoll box -MG
    elif basename(filename)[0:3]=='Fco':
        if args.min > 1e4:
        ...
        print "Avg: "+str(np.sum(data1)/(200**3))

        print "data1 has shape: ", data1.shape
        print "MAX_OF_MAR: "+str(np.max(data1))
        print "MIN_OF_MAR: "+str(np.min(data1))
        print "MEAN_OF_MAR: "+str(np.mean(data1))
        print "MEDIAN_OF_MAR: "+str(np.median(data1))

        print "WHERE:", np.size(np.where(data1==1.0)[0])

        data1_not_one = data1[np.where(data1!=1.0)]
        print "data1_not_one has max:", np.max(data1_not_one)
        print "data1_not_one has min:", np.min(data1_not_one)

        minrange = 0
        maxrange = 1
        cmap = LinearSegmentedColormap.from_list('mymap', ['darkblue', 'green', 'yellow', 'red'])
        frame1 = plt.gca()
        frame1.axes.get_xaxis().set_ticks([])
        frame1.axes.get_yaxis().set_ticks([])
        frame1.set_xlabel(r'${\rm\longleftarrow %s \longrightarrow}$'%(label), fontsize=20)
        c_dens = sub_fig.imshow(slice,cmap=cmap)
        c_dens.set_clim(vmin=minrange,vmax=maxrange)
        c_bar = fig.colorbar(c_dens, orientation='vertical')
        c_bar.set_label(r'${\rm F_{collapsed}}$', fontsize=24, rotation=-90, labelpad=32) #to set later
        tick_array = np.linspace(minrange, maxrange, 3) #to set later

    # check if it is an MAR box -GS
    elif basename(filename)[0:3]=='MAR':
        if args.min > 1e4:
            minrange = -2 #to set later
        if args.max < -1e4:
            maxrange = 0 #to set later
        ...
        print "SFRD_from_MAR: ", np.sum(data1)/np.prod(data1.shape)
        print "MEAN SFRD_from_MAR: ", np.mean(data1.flatten())
        print "STDEV SFRD_from_MAR: ", np.std(data1.flatten())
        minrange = -5
        maxrange = 0
        cmap = LinearSegmentedColormap.from_list('mymap', ['darkblue', 'green', 'yellow', 'red'])
        frame1 = plt.gca()
        ...
        c_dens.set_clim(vmin=minrange,vmax=maxrange)
        c_bar = fig.colorbar(c_dens, orientation='vertical')
        c_bar.set_label(r'${\rm \log(M_\odot yr^{-1})}$', fontsize=24, rotation=-90, labelpad=32) #to set later
        tick_array = np.linspace(minrange, maxrange, 5) #to set later
    ...
    # check if it is an MAR box -GS
        ...
        print "SFRD_from_MAR: ", np.sum(data1)/np.prod(data1.shape), np.log10(np.sum(data1)/np.prod(data1.shape))
        print "MEAN SFRD_from_MAR: ", np.mean(data1.flatten())
        print "STDEV SFRD_from_MAR: ", np.std(data1.flatten())
        minrange = -5
        maxrange = 0
    ...
    # check if it is an MAR box -GS
        ...
        print "SMD_from_MAR: ", np.sum(data1)/np.prod(data1.shape), np.log10(np.sum(data1)/np.prod(data1.shape))
        minrange = -5
        maxrange = 0
        cmap = LinearSegmentedColormap.from_list('mymap', ['darkblue', 'green', 'yellow', 'red'])
        frame1 = plt.gca()
    ...
    # check if it is an Halpha box -GS
    elif basename(filename)[0:3]=='Hal':
        if args.min > 1e4:
            minrange = -2 #to set later
        if args.max < -1e4:
            maxrange = 0 #to set later
        ...
        print "Mean Ha intensity: ", np.sum(data1)/np.prod(data1.shape)/1.0e-23, np.log10(np.sum(data1)/np.prod(data1.shape)/1.0e-23)
        print "STDEV Ha intensity: ", np.std(data1)/1.0e-23
        #np.savez('./Halpha_Ene_Box.npz', **{'box': data1/1.0e-23})
        minrange = -5
        maxrange = 0
        cmap = LinearSegmentedColormap.from_list('mymap', ['darkblue', 'green', 'yellow', 'red'])
        frame1 = plt.gca()
        ...
        frame1.axes.get_yaxis().set_ticks([])
        frame1.set_xlabel(r'${\rm\longleftarrow %s \longrightarrow}$'%(label), fontsize=20)
        c_dens = sub_fig.imshow(slice+23.,cmap=cmap)
        c_dens.set_clim(vmin=minrange,vmax=maxrange)
        c_bar = fig.colorbar(c_dens, orientation='vertical')
        c_bar.set_label(r'${\rm \log(M_\odot yr^{-1})}$', fontsize=24, rotation=-90, labelpad=32) #to set later
        tick_array = np.linspace(minrange, maxrange, 5) #to set later
    ...
    # check if it is an Halpha box -GS
    elif basename(filename)[8:15]=='HI6563A':
        if args.min > 1e4:
            minrange = -2 #to set later
        if args.max < -1e4:
            maxrange = 0 #to set later
        ...
        np.savez('./HI6563A_z005_Mom_Box.npz', **{'box': data1/1.0e-23})

        minrange = -0.5#np.min(slice)+23+1.5
        maxrange = 0.5#np.max(slice)+23-1.5
        #cmap = LinearSegmentedColormap.from_list('mymap', ['darkblue', 'green', 'yellow', 'red'])
        cmap = LinearSegmentedColormap.from_list('mycmap', ['white','black'])
        frame1 = plt.gca()
        frame1.axes.get_xaxis().set_ticks([])
        ...
        c_dens.set_clim(vmin=minrange,vmax=maxrange)
        c_bar = fig.colorbar(c_dens, orientation='vertical')
        c_bar.set_label(r'${\rm \log(I_\mathrm{H\alpha}/\mathrm{[Jy/sr]})}$', fontsize=24, rotation=-90, labelpad=32) #to set later
        tick_array = np.linspace(minrange, maxrange, 5) #to set later
    ...
    # check if it is an Halpha box -GS
        ...
        print "Mean OIII intensity: ", np.sum(data1)/np.prod(data1.shape)/1.0e-23, np.log10(np.sum(data1)/np.prod(data1.shape)/1.0e-23)

        np.savez('./HI6563A_z010_Mom_Box.npz', **{'box': data1/1.0e-23})
    ...
    # check if it is a mass box -MG
    elif basename(filename)[0:3]=='mas':
        if args.min > 1e4:
            minrange = -2 #to set later
        if args.max < -1e4:
            maxrange = 0 #to set later
        ...
        c_dens.set_clim(vmin=minrange,vmax=maxrange)
        c_bar = fig.colorbar(c_dens, orientation='vertical')
        c_bar.set_label(r'${\rm \log(M_\odot)}$', fontsize=24, rotation=-90, labelpad=32) #to set later
        tick_array = np.linspace(minrange, maxrange, 4) #to set later

    # check if it is a collapsed mass box -MG
    elif basename(filename)[0:3]=='col':
        if args.min > 1e4:
            minrange = -2 #to set later
        if args.max < -1e4:
            maxrange = 0 #to set later
        ...
        print "Collapsed mass density: "+str(np.sum(data1)/((1.0*length)**3))+"Msun Mpc^-3"
        minrange = 2
        maxrange = 12
        cmap = LinearSegmentedColormap.from_list('mymap', ['darkblue', 'green', 'yellow', 'red'])
        frame1 = plt.gca()
        ...
        c_bar = fig.colorbar(c_dens, orientation='vertical')
        c_bar.set_label(r'${\rm \log(M_\odot)}$', fontsize=24, rotation=-90, labelpad=32) #to set later
        tick_array = np.linspace(minrange, maxrange, 6) #to set later

    # check if it is a stellar mass box -MG
    elif basename(filename)[0:3]=='ste':
        if args.min > 1e4:
            minrange = 1e8 #to set later
        if args.max < -1e4:
            maxrange = 1e15 #to set later
        print "SMD (slice): "+str(np.log10(np.sum(slice)/((1.0*length)**2)))
        slice = np.log10(slice)
        print "Min: "+str(np.min(slice))
        print "Max: "+str(np.max(slice))
        print "SMD (box): "+str(np.log10(np.sum(data1)/((1.0*length)**3)))
        minrange = 0
        maxrange = 10
        cmap = LinearSegmentedColormap.from_list('mymap', ['darkblue', 'green', 'yellow', 'red'])
        norm = MidpointNormalize(midpoint=1.5) #to set later
        frame1 = plt.gca()
        frame1.axes.get_xaxis().set_ticks([])
        frame1.axes.get_yaxis().set_ticks([])
        frame1.set_xlabel(r'${\rm\longleftarrow %s \longrightarrow}$'%(label), fontsize=20)
    ...
    # check if it is a stellar mass box -MG
        ...
        if args.max < -1e4:
            maxrange = 1e15 #to set later
        slice = np.divide(slice, 0.014)
        slice = np.log10(slice)
        print "Min: "+str(np.min(slice))
        print "Max: "+str(np.max(slice))
        #print "Avg: "+str(np.log10(np.divide(np.sum(data1)/(200**3),0.014)))
        print "Avg: "+str(np.log10(np.mean(data1)/0.014))
        print "Over: "+str(len(slice[np.where(slice >= 0)]))
        minrange = -9
        maxrange = 1
        cmap = LinearSegmentedColormap.from_list('mymap', ['darkblue', 'green', 'yellow', 'red'])
        norm = MidpointNormalize(midpoint=1.5) #to set later
        ...
        c_dens.set_clim(vmin=minrange,vmax=maxrange)
        c_bar = fig.colorbar(c_dens, orientation='vertical')
        c_bar.set_label(r'${\rm \log(z/z_{\odot})}$', fontsize=24, rotation=-90, labelpad=32) #to set later
        tick_array = np.linspace(minrange, maxrange, 3) #to set later

    # check if it is a lya background box -MG
    elif basename(filename)[0:3]=='Hal':
        if args.min > 1e4:
            minrange = 1e8 #to set later
        if args.max < -1e4:
        ...
        c_dens.set_clim(vmin=minrange,vmax=maxrange)
        c_bar = fig.colorbar(c_dens, orientation='vertical')
        c_bar.set_label(r'${\rm \log(erg\ s^{-1} cm^{-2} Hz^{-1} sr^{-1})}$', fontsize=20, rotation=-90, labelpad=32) #to set later
        tick_array = np.linspace(minrange, maxrange, 4) #to set later


    # check if it is a lya background box -MG
    elif basename(filename)[0:5]=='Lyasf':
        if args.min > 1e4:
            minrange = 1e8 #to set later
        if args.max < -1e4:
            maxrange = 1e15 #to set later
        #slice = np.multiply(slice, 2.466e15)
        #slice = np.divide(slice, 1+15.15)
        slice = np.log10(slice)
        print "Min: "+str(np.min(slice))
        print "Max: "+str(np.max(slice))
        print "Avg: "+str(np.mean(data1))
        minrange = -21
        maxrange = -13
        cmap = LinearSegmentedColormap.from_list('mymap', ['darkblue', 'green', 'yellow', 'red'])
        norm = MidpointNormalize(midpoint=1.5) #to set later
    ...
    # check if it is a lya background box -MG
    elif basename(filename)[0:6]=='Lyabkg':
        if args.min > 1e4:
            minrange = 1e8 #to set later
        if args.max < -1e4:
            maxrange = 1e15 #to set later
    ...
    # check if it is a lya background box -MG
    elif basename(filename)[0:6]=='Lyaigm':
        if args.min > 1e4:
            minrange = 1e8 #to set later
        if args.max < -1e4:
            maxrange = 1e15 #to set later
