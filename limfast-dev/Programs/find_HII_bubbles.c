...
#include "compute_all_lines.c" //-GS
...
void init_21cmMC_arrays() { // defined in Cosmo_c_files/ps.c
    ...
    Overdense_spline_MAR = calloc(NSFR_high,sizeof(float)); //-GS
    Overdense_spline_BURST = calloc(NSFR_high,sizeof(float)); //-LMR
    Fcoll_spline_SFR = calloc(NSFR_high,sizeof(float));
    Fcoll_spline_SFR_fs = calloc(NSFR_high,sizeof(float)); //-MG
    Fcoll_spline_MAR = calloc(NSFR_high,sizeof(float)); //-GS
    Fcoll_spline_BURST = calloc(NSFR_high,sizeof(float)); //-LMR
    ...
    second_derivs_MAR = calloc(NSFR_high,sizeof(float)); //-GS
    second_derivs_BURST = calloc(NSFR_high,sizeof(float)); //-GS
...
void destroy_21cmMC_arrays() {
    ...
    free(Overdense_spline_MAR); //-GS
    free(Overdense_spline_BURST); //-LMR
    free(Fcoll_spline_SFR);
    free(Fcoll_spline_SFR_fs); //-MG
    free(Fcoll_spline_MAR); //-GS
    free(Fcoll_spline_BURST); //-LMR
    if(USE_GENERAL_SOURCES)
    {
    ...
    free(second_derivs_MAR); //-GS
    free(second_derivs_BURST); //-LMR
    free(xi_SFR);
    free(wi_SFR);
}
...
int main(int argc, char ** argv){
  ...
  float f_coll_crit, pixel_volume,  density_over_mean, erfc_num, erfc_denom, erfc_denom_cell, res_xH, Splined_Fcoll, Splined_Fcoll_uni, Splined_Fcoll_fs, Splined_Fcoll_MAR, Splined_Fcoll_halolumburst;
  ...
  double mean_f_coll_st_MAR, ST_over_PS_MAR, f_coll_MAR; // -GS
  ...
  float Lya_pop2_full_table[DIM_POP2_METALLICITY][DIM_POP2_ION_PARAM], Ha_pop2_full_table[DIM_POP2_METALLICITY][DIM_POP2_ION_PARAM],
        O3_pop2_full_table[DIM_POP2_METALLICITY][DIM_POP2_ION_PARAM], O2_pop2_full_table[DIM_POP2_METALLICITY][DIM_POP2_ION_PARAM]; //-GS
  ...
  *error_message = '\0';
  ...
  //printf("\n\n- ST_over_PS_uni: %f\n\n", ST_over_PS_uni);
  //printf("\n\n- CHECK OIII: %e\n\n", get_O3_luminosity(O3_pop2_full_table, -1., -2.));
  //printf("\n\n- CHECK OII: %e\n\n", get_O2_luminosity(O2_pop2_full_table, -1., -2.));
...
/*  if ((fabs(ALPHA_STAR) > FRACT_FLOAT_ERR) ||
  ...
  overden_box = (float *) malloc(sizeof(float)*HII_TOT_FFT_NUM_PIXELS);;
  MAR = (float *) malloc(sizeof(float)*HII_TOT_FFT_NUM_PIXELS);
  lineburstbox = (float *) malloc(sizeof(float)*HII_TOT_FFT_NUM_PIXELS);
  BURST = (float *) malloc(sizeof(float)*HII_TOT_FFT_NUM_PIXELS);
  sfrd_box = (float *) malloc(sizeof(float)*HII_TOT_FFT_NUM_PIXELS);
  smd_box = (float *) malloc(sizeof(float)*HII_TOT_FFT_NUM_PIXELS);
  fstar_box = (float *) malloc(sizeof(float)*HII_TOT_FFT_NUM_PIXELS);
  metallicity = (float *) malloc(sizeof(float)*HII_TOT_FFT_NUM_PIXELS);
  metallicity_cm = (float *) malloc(sizeof(float)*HII_TOT_FFT_NUM_PIXELS);
  ...
  H1_1216A_sf = (float *) malloc(sizeof(double)*HII_TOT_FFT_NUM_PIXELS); // -GS
 ...
 // }
  ...
  //fprintf(stderr, "The value of mean_f_coll_st_fs at redshift %f is %e\n", REDSHIFT, mean_f_coll_st_fs);
  //fprintf(stderr, "The value of mean_f_coll_st_MAR at redshift %f is %e\n", REDSHIFT, mean_f_coll_st_MAR);
  //fprintf(stderr, "The value of mean_f_coll_st_LineLum at redshift %f is %e\n", REDSHIFT, mean_f_coll_st_LineLum);
 ...
 CLEANUP: if (*error_message != '\0'){
    ...
    Splined_Fcoll_MAR = Splined_Fcoll;
    Splined_Fcoll_halolumburst = Splined_Fcoll; //-LMR
    // - RM
    /*
    if(USE_GENERAL_SOURCES)
    {
      ...
      if(USE_GENERAL_SOURCES) {
        ...
        zetaSpline_SFR(density_over_mean - 1,&(Splined_zeta));
            ...
            Splined_Fcoll_uni = splined_erfc(erfc_num/erfc_denom);
              ...
              //Splined_Fcoll_MAR = Splined_Fcoll_uni;
            }
      		}
  ...
  f_coll_MAR /= (double) HII_TOT_NUM_PIXELS;
  f_coll_halolumburst /= (double) HII_TOT_NUM_PIXELS;
  ...
  //printf("Tell me ST_over_PS_uni/ST_over_PS_MAR: %e/%e\n", ST_over_PS_uni, ST_over_PS_MAR);
    ...
    if (!(F = fopen(filename, "rb"))){
      ...
      for (x=0; x<HII_DIM; x++) {
        for (y=0; y<HII_DIM; y++) {
          for (z=0; z<HII_DIM; z++) {
            if (fread((float *)metallicity_cm + HII_R_FFT_INDEX(x,y,z), sizeof(float), 1, F)!=1){
              strcpy(error_message, "find_HII_bubbles.c: Read error occured while reading metallicity box!\n");
              compute_seds = 0;
              goto CLEANUP;
            }
    ...
    //read in last SMD box to add to (if applicable) -GS
    sprintf(filename, "../Boxes/SMD_z%06.2f_HIIfilter%i_RHIImax%.0f_%i_%.0fMpc", PREV_REDSHIFT, HII_FILTER, MFP, HII_DIM, BOX_LEN);
    F = fopen(filename, "rb");
    if (!(F = fopen(filename, "rb"))){
      fprintf(stderr, "Error opening file %s for reading.\n", filename);
      fprintf(LOG, "Error opening file %s for reading.\n", filename);
      ...
      for (x=0; x<HII_DIM; x++) {
        for (y=0; y<HII_DIM; y++) {
          for (z=0; z<HII_DIM; z++) {
            if (fread((float *)smd_box + HII_R_FFT_INDEX(x,y,z), sizeof(float), 1, F)!=1){
              strcpy(error_message, "find_HII_bubbles.c: Read error occured while reading metallicity box!\n");
              compute_seds = 0;
              goto CLEANUP;
            }
    ...
    for (x=0; x<HII_DIM; x++){
      for (y=0; y<HII_DIM; y++){
        for (z=0; z<HII_DIM; z++){
          ...
          sfrd_box[ct] = MAR[ct] * ST_over_PS_MAR; // get normalized SFRD -GS
          //smd_box[ct] += sfrd_box[ct] * val; // get SMD from SFRD directly -GS
          //fstar_box[ct] = Fcoll_fs[ct] * ST_over_PS_fs; // get fstar box -GS
        ...
        for (x=0; x<HII_DIM; x++){
          for (y=0; y<HII_DIM; y++){
            for (z=0; z<HII_DIM; z++){
              ...
              ct = HII_R_FFT_INDEX(x,y,z);
              metal_val = (float) log10(metallicity[ct] / Z_sun); //units of log(z/z_sun);
    ...
    if (INHOMO_RECO){
     ...
     // overdensity box -GS
     sprintf(filename, "../Boxes/overden_z%06.2f_HIIfilter%i_RHIImax%.0f_%i_%.0fMpc", REDSHIFT, HII_FILTER, MFP, HII_DIM, BOX_LEN);
     if (!(F = fopen(filename, "wb"))) {
       sprintf(error_message, "find_HII_bubbles.c: ERROR: unable to open file for writing MAR box.\n");
       goto CLEANUP;
     }
     else {
       for (x = 0; x < HII_DIM; x++) {
         for (y = 0; y < HII_DIM; y++) {
           for (z = 0; z < HII_DIM; z++) {
             if (fwrite(overden_box + HII_R_FFT_INDEX(x,y,z), sizeof(float), 1, F) != 1) {
                sprintf(error_message, "find_HII_bubbles.c: Write error occured while writing MAR box.\n");
                goto CLEANUP;
             }
           }
         }
     ...
     // MAR box -GS
     sprintf(filename, "../Boxes/MAR_z%06.2f_HIIfilter%i_RHIImax%.0f_%i_%.0fMpc", REDSHIFT, HII_FILTER, MFP, HII_DIM, BOX_LEN);
     if (!(F = fopen(filename, "wb"))) {
       sprintf(error_message, "find_HII_bubbles.c: ERROR: unable to open file for writing MAR box.\n");
       goto CLEANUP;
     }
       ...
       for (x = 0; x < HII_DIM; x++) {
         for (y = 0; y < HII_DIM; y++) {
           for (z = 0; z < HII_DIM; z++) {
             if (fwrite(MAR + HII_R_FFT_INDEX(x,y,z), sizeof(float), 1, F) != 1) {
                sprintf(error_message, "find_HII_bubbles.c: Write error occured while writing MAR box.\n");
                goto CLEANUP;
             }
           }
     ...
     // SFRD box (i.e., MAR box multiplied by the ST_over_PS_MAR) -GS
     ...
     // SMD box -GS
     sprintf(filename, "../Boxes/SMD_z%06.2f_HIIfilter%i_RHIImax%.0f_%i_%.0fMpc", REDSHIFT, HII_FILTER, MFP, HII_DIM, BOX_LEN);
     if (!(F = fopen(filename, "wb"))) {
       ...
       for (x = 0; x < HII_DIM; x++) {
         for (y = 0; y < HII_DIM; y++) {
           for (z = 0; z < HII_DIM; z++) {
             if (fwrite(smd_box + HII_R_FFT_INDEX(x,y,z), sizeof(float), 1, F) != 1) {
     ...
     /*// metallicity (cosmic mean) box -GS
     sprintf(filename, "../Boxes/metallicitycm_z%06.2f_HIIfilter%i_RHIImax%.0f_%i_%.0fMpc", REDSHIFT, HII_FILTER, MFP, HII_DIM, BOX_LEN);
     if (!(F = fopen(filename, "wb"))) {
       sprintf(error_message, "find_HII_bubbles.c: ERROR: unable to open file for writing metallicity box.\n");
       goto CLEANUP;
     }
       ...
       for (x = 0; x < HII_DIM; x++) {
         for (y = 0; y < HII_DIM; y++) {
           for (z = 0; z < HII_DIM; z++) {
             if (fwrite(metallicity_cm + HII_R_FFT_INDEX(x,y,z), sizeof(float), 1, F) != 1) {
                sprintf(error_message, "find_HII_bubbles.c: Write error occured while writing metallicity box.\n");
                goto CLEANUP;
             }
           }
     ...
     if (USE_CUSTOM_SEDS && compute_seds) {
           ...
           for (y = 0; y < HII_DIM; y++) {
             for (z = 0; z < HII_DIM; z++) {
               if (fwrite(lineburstbox + HII_R_FFT_INDEX(x,y,z), sizeof(float), 1, F) != 1) {
                  sprintf(error_message, "find_HII_bubbles.c: Write error occured while writing Halpha_sf_perhalo box.\n");
                  goto CLEANUP;
               }
             }
           }
      ...
      // HeII_sf box -MG
       ...
       if (!(F = fopen(filename, "wb"))) {
         sprintf(error_message, "find_HII_bubbles.c: ERROR: unable to open file for writing LineLum_HI1216A box.\n");
         goto CLEANUP;
       }
       else {
         for (x = 0; x < HII_DIM; x++) {
           for (y = 0; y < HII_DIM; y++) {
             for (z = 0; z < HII_DIM; z++) {
               if (fwrite(H1_1216A_sf + HII_R_FFT_INDEX(x,y,z), sizeof(float), 1, F) != 1) {
                  sprintf(error_message, "find_HII_bubbles.c: Write error occured while writing LineLum_HI1216A box.\n");
                  goto CLEANUP;
               }
             }
           }
       ...
       // New Ha box - GS
       sprintf(filename, "../Boxes/LineLum_HI6563A_z%06.2f_HIIfilter%i_RHIImax%.0f_%i_%.0fMpc", REDSHIFT, HII_FILTER, MFP, HII_DIM, BOX_LEN);
       if (!(F = fopen(filename, "wb"))) {
         sprintf(error_message, "find_HII_bubbles.c: ERROR: unable to open file for writing LineLum_HI6563A box.\n");
         goto CLEANUP;
       }
       else {
         for (x = 0; x < HII_DIM; x++) {
           for (y = 0; y < HII_DIM; y++) {
             for (z = 0; z < HII_DIM; z++) {
               if (fwrite(H1_6563A_sf + HII_R_FFT_INDEX(x,y,z), sizeof(float), 1, F) != 1) {
                  sprintf(error_message, "find_HII_bubbles.c: Write error occured while writing LineLum_HI6563A box.\n");
                  goto CLEANUP;
               }
             }
           }
       ...
       // New OIII box - GS
       sprintf(filename, "../Boxes/LineLum_OIII5007A_z%06.2f_HIIfilter%i_RHIImax%.0f_%i_%.0fMpc", REDSHIFT, HII_FILTER, MFP, HII_DIM, BOX_LEN);
       if (!(F = fopen(filename, "wb"))) {
         sprintf(error_message, "find_HII_bubbles.c: ERROR: unable to open file for writing LineLum_OIII5007A box.\n");
         goto CLEANUP;
       }
       else {
         for (x = 0; x < HII_DIM; x++) {
           for (y = 0; y < HII_DIM; y++) {
             for (z = 0; z < HII_DIM; z++) {
               if (fwrite(O3_5007A_sf + HII_R_FFT_INDEX(x,y,z), sizeof(float), 1, F) != 1) {
                  sprintf(error_message, "find_HII_bubbles.c: Write error occured while writing LineLum_OIII5007A box.\n");
                  goto CLEANUP;
               }
             }
           }
       ...
       // New OII box - GS
       sprintf(filename, "../Boxes/LineLum_OII3727A_z%06.2f_HIIfilter%i_RHIImax%.0f_%i_%.0fMpc", REDSHIFT, HII_FILTER, MFP, HII_DIM, BOX_LEN);
       if (!(F = fopen(filename, "wb"))) {
         sprintf(error_message, "find_HII_bubbles.c: ERROR: unable to open file for writing LineLum_OII3727A box.\n");
         goto CLEANUP;
       }
       else {
         for (x = 0; x < HII_DIM; x++) {
           for (y = 0; y < HII_DIM; y++) {
             for (z = 0; z < HII_DIM; z++) {
               if (fwrite(O2_3727A_sf + HII_R_FFT_INDEX(x,y,z), sizeof(float), 1, F) != 1) {
                  sprintf(error_message, "find_HII_bubbles.c: Write error occured while writing LineLum_OII3727A box.\n");
                  goto CLEANUP;
               }
             }
           }
