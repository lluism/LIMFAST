...
#define CONVERT_TO_DELTA (int) 0 /* 1= convert the field to a zero-mean delta; be careful not to do this with fields which are already zero mean */
...
int main(int argc, char ** argv){
  ...
  double dvdx, ave, new_ave, *p_box, *pk_box, *k_ave;
  ...
  pk_box = (double *)malloc(sizeof(double)*NUM_BINS);
  ...
  for (n_x=0; n_x<HII_DIM; n_x++){
	...
	while (k_ceil < k_max){
	  ...
	  if ((k_mag>=k_floor) && (k_mag < k_ceil)){
	    in_bin_ct[ct]++;
      pk_box[ct] += pow(cabs(deltax[HII_C_INDEX(n_x, n_y, n_z)]), 2) / (VOLUME+0.0);
  ...
  for (ct=1; ct<NUM_BINS; ct++){ //[k] [pk] [deltak] [error?]
    fprintf(F, "%e\t%e\t%e\t%e\n", k_ave[ct]/(in_bin_ct[ct]+0.0), pk_box[ct]/(in_bin_ct[ct]+0.0), p_box[ct]/(in_bin_ct[ct]+0.0), p_box[ct]/(in_bin_ct[ct]+0.0)/sqrt(in_bin_ct[ct]+0.0));
  }
  fclose(F);

  /****** END POWER SPECTRUM STUFF   ************/

  free(p_box); free(pk_box); free(k_ave); free(in_bin_ct);
