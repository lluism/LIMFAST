...
int main(int argc, char ** argv){
      ...
      for (j=0; j<HII_DIM; j++){
          ...
          for(k=0;k<HII_DIM;k++) {
            ...
            for (k=0; k<HII_DIM; k++){
                ...
                if(fabs(I_lim_rsd[HII_R_INDEX(i,j,k)]) >= ave_I_lim_rsd*FRACT_FLOAT_ERR) {
  ...
  ave_I_lim_rsd /= (float)HII_TOT_NUM_PIXELS;
  fprintf(stderr, "CHECK AFTER: ave_I_lim_rsd = %.3e, ave_I_lim_rsd = %.3e\n", ave_I_lim, ave_I_lim_rsd);

  // now write out the line intensity box with velocity correction
  sprintf(filename, "../Boxes/I_%s_v%i_z%06.2f_HIIfilter%i_RHIImax%.0f_%i_%.0fMpc", argv[3], VELOCITY_COMPONENT, REDSHIFT, HII_FILTER, R_BUBBLE_MAX, HII_DIM, BOX_LEN);
  F = fopen(filename, "wb");
