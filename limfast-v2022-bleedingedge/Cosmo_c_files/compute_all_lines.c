...
#define DIM_POP2_METALLICITY (int) 13
#define DIM_POP2_ION_PARAM (int) 7
#define DIM_LUM (int) 91
...
#define C2_POP2_TABLE_FILENAME (const char *) "../External_tables/line_tables_bpass/LlineTbl_POP2_CII157m_Continuous.dat"
#define CO10_POP2_TABLE_FILENAME (const char *) "../External_tables/line_tables_bpass/LlineTbl_POP2_CO2600m_Continuous.dat"
#define O3FIR_POP2_TABLE_FILENAME (const char *) "../External_tables/line_tables_bpass/LlineTbl_POP2_OIII88m_Continuous.dat"

// Metallicity sampling points
double Z_VALUES[]={9.999e-6, 1.0e-4, 1.0e-3, 2.0e-3, 3.0e-3, 4.0e-3, 6.0e-3, 8.0e-3, 1.0e-2, 1.4e-2, 2.0e-2, 3.0e-2, 4.001e-2};
...
static double rdata_LUM_LYA_flat[DIM_LUM], rdata_LUM_HA_flat[DIM_LUM], rdata_LUM_HB_flat[DIM_LUM], rdata_LUM_HE2_flat[DIM_LUM],
              ...
              rdata_LUM_CO10_flat[DIM_LUM], rdata_LUM_O3FIR_flat[DIM_LUM];

static gsl_spline2d *spline_lya_lum, *spline_ha_lum, *spline_hb_lum, *spline_he2_lum, *spline_o3_lum,
                    *spline_o2s_lum, *spline_o2l_lum, *spline_c2_lum, *spline_co10_lum, *spline_o3fir_lum;
...
int init_pop2_full_tables(float lya_table_pop2[DIM_POP2_METALLICITY][DIM_POP2_ION_PARAM],
                          ...
                          float o3fir_table_pop2[DIM_POP2_METALLICITY][DIM_POP2_ION_PARAM]) {
  int i, ii, jj, kk;
  FILE *F;
  ...
  //initialize OIII 88um table
  if (!(F = fopen(O3FIR_POP2_TABLE_FILENAME, "r"))) {
    fprintf(stderr, "Unable to open file: LlineTbl_POP2_OIII88m.dat for reading\nAborting\n");
    return -1;
  }
  for (i = 0; i < DIM_POP2_METALLICITY; i++) {
    fscanf(F, "%*[^\n]\n");
    fscanf(F, "%e %e %e %e %e %e %e", &o3fir_table_pop2[i][0], &o3fir_table_pop2[i][1], &o3fir_table_pop2[i][2], &o3fir_table_pop2[i][3],
                                      &o3fir_table_pop2[i][4], &o3fir_table_pop2[i][5], &o3fir_table_pop2[i][6]);
  }
  fclose(F);
  ...
  for (jj = 0; jj < DIM_POP2_ION_PARAM; jj++) {
    for (ii = 0; ii < DIM_POP2_METALLICITY; ii++) {
      ...
      rdata_LUM_O3FIR_flat[kk] = o3fir_table_pop2[ii][jj];
    }
  }
  ...
  spline_o3fir_lum = gsl_spline2d_alloc(T, nx, ny);
  ...
  gsl_spline2d_init(spline_o3fir_lum, rdata_Z, rdata_U, rdata_LUM_O3FIR_flat, nx, ny);
...
float get_o3fir_luminosity(float Z, int logU) {
  return gsl_spline2d_eval(spline_o3fir_lum, Z, logU, xacc_lum, yacc_lum);
}
